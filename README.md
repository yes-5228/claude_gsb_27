...
| 模块 | 页面/入口 | 主要能力 |
| --- | --- | --- |
| 总览看板 | `/` | 核心指标卡（含本月漏巡）、巡查与问题趋势、整改状态/分类/严重程度分布、区域运行情况、重点关注公厕、最新问题与巡查、月度考核 |
| 公厕台账 | `/restrooms`、`/restrooms/:id` | 台账增删改查、区域与状态筛选、公厕详情（档案 + 历史巡查 + 历史问题 + 状态记录）、开放状态变更联动、关联数据删除保护 |
| 保洁巡查 | `/inspections` | 8 项检查项打分、自动折算百分制得分与等级、班次/日期/结论筛选、巡查详情、一键转问题上报；停用公厕不再生成巡查任务 |
| 问题上报 | `/issues`、`/issues/:id` | 问题上报（可关联巡查记录）、分类/程度/期限、整改流程流转、整改轨迹时间线、超期预警、追加跟进记录、停用期间期限顺延 |
...
| POST | `/restrooms/{id}/status` | 变更开放状态（联动停用区间与整改期限顺延，留痕） |
| GET | `/restrooms/{id}/status-events` | 状态变更留痕（停用与恢复） |
| GET | `/restrooms/{id}/suspensions` | 停用区间列表 |
| GET/POST | `/rules/deadline-extensions` | 整改期限顺延规则查询 / 新建（带生效时间） |
...
| GET | `/stats/overview` | 核心指标（含本月漏巡） |
| GET | `/stats/dashboard` | 看板聚合数据（趋势、分布、区域、排行、最新记录） |
| POST | `/stats/assessments/generate` | 生成月度考核快照（已生成的不重复、不回溯） |
| GET | `/stats/assessments` | 月度考核列表（可按月份过滤） |
| GET | `/stats/assessments/months` | 已生成考核的月份列表 |
...
- **状态联动**：公厕在 `正常开放 / 维修中 / 暂停使用` 间变更时与巡查、整改联动——转入维修/暂停后不再生成新的巡查任务；同时开启一段「停用区间」，恢复开放时关闭。停用与恢复都会写入状态变更留痕（原状态、新状态、原因、操作人、时间）。
- **整改期限顺延**：恢复开放时，按停用开启时生效的顺延规则，对该公厕仍未闭环且设了期限的问题顺延期限，并在整改流水里写明原因。顺延规则带 `effective_from` 生效时间，只影响生效之后发生的停用判定；每段停用区间对同一问题只顺延一次（反复切换不重复、不漏项）。规则支持「按实际停用时长顺延」或「固定顺延 N 天」。
- **漏巡统计**：每座正常开放公厕每日应巡 2 次，应巡次数按开放天数折算，停用区间不计入应巡，因此停用期间的空白不会被算成漏巡。
- **月度考核**：按公厕生成月度快照（开放天数、应巡/实巡/漏巡、均分、问题上报与闭环、超期数），生成后冻结，后续状态变化与期限顺延不回溯修改；同一公厕同一月份只生成一次。
...
```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、流程流转、统计、状态联动）
